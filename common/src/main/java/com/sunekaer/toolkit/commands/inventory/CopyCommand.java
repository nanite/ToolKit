...
public class CopyCommand {
    ...
    private static int copy(CommandContext<CommandSourceStack> context, @Nullable String outputType) throws CommandSyntaxException {
        ...
        var outputString = computedOutputType.function.apply(nonEmptyItems);

        source.sendSuccess(Component.translatable("commands.toolkit.clipboard.copied"), true);
        Handler.CHANNEL.sendToPlayer(player, new SetCopy(outputString));

        return 1;
    }

    enum OutputType {
        KUBEJS(4,"kubejs", (items) -> {
        ...
        KUBEJS_NATIVE(5,"kubejs_native", (items) -> {
            StringBuilder builder = new StringBuilder();
            builder.append("[").append(CommandUtils.NEW_LINE);
        ...
        JSON(3, "json", (items) -> {
            var output = items.stream().map(stack -> {
                ...
                if (nbt.contains("tag")) {
                    return Map.of(
                            "item", itemName,
                            "nbt", Objects.requireNonNull(nbt.get("tag"))
                    );
                }
         ...
         */
        CRAFTTWEAKER(5, "crafttweaker", (items) -> {
            StringBuilder builder = new StringBuilder();
            builder.append("[").append(CommandUtils.NEW_LINE);
        ...
        final int order;
        final String name;
        final Function<List<ItemStack>, String> function;

        OutputType(int order, String name, Function<List<ItemStack>, String> function) {
            this.order = order;
            this.name = name;
